...
private:
  const uint8_t  RST = 7; // Reset BIT
  const int SOCKETS = 8;

public:
  const uint16_t SSIZE = 2048; // Max Tx buffer size
private:
  const uint16_t RSIZE = 2048; // Max Rx buffer size

private:
  // could do inline optimizations
  inline void initSS()  { pinMode(_SPI_CS, OUTPUT); }
  inline void setSS()   {  digitalWrite(_SPI_CS, LOW); }
  inline void resetSS() {  digitalWrite(_SPI_CS, HIGH); }
};

// extern W5500Class w5500;  // needs instance
